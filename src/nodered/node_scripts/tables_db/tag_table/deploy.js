...
for (const change of msg.dashboard.history) {
    ...
    if (oldItem && !newItem) {
        ...
        commands.push(`ALTER TABLE IF EXISTS "${processSchema}"."${processTable}" DROP COLUMN IF EXISTS ${oldName};`);
        ...
        commands.push(`ALTER TABLE IF EXISTS "${processSchema}"."${processTable}" ADD COLUMN IF NOT EXISTS ${newItem.name} ${data_type};`);
        ...
        if (oldItem.name !== newItem.name) {
            commands.push(`ALTER TABLE IF EXISTS "${processSchema}"."${processTable}" RENAME COLUMN ${oldItem.name} TO ${newItem.name};`);
        }
        if (oldItem.data_type !== newItem.data_type) {
            const data_type = data_type_dict[newItem.data_type] ? data_type_dict[newItem.data_type] : 'TEXT';
            commands.push(`ALTER TABLE IF EXISTS "${processSchema}"."${processTable}" ALTER COLUMN ${newItem.name} TYPE ${data_type};`);
        }
    }
}
